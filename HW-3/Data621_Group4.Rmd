...
#### From the above boruta feature selection chart, all feature variables are confirmed important variables. but chas is clearly the last one, and very close to shadow (random) variables, so it's contribution to the model is questionable.
...
#### Use all original 12 variables in the dataset to fit the model without any interation and transformed variables.
...
#### Above metrics are better than 8 variable base model, it is expected as more information was brought in the model by additional variables, the accuracy is close to the 2nd model above with interaction term. 
...
#### Above metrics are better than the 4th model that uses all variables in the model, it proves that removing the tentative variable chas improves the model performance; the accuracy and precision both beat the first 2 models above, but slightly below the model number 3 that has 8 origianl variables plus 3 interaction and transformed variables.
...
### **Discussion. The best 2 models are compared as the bar chart shows below. Both models have 11 variables. The interaction term and transformed variables brought higher accuracy, AUC, and recall to the model, but precision value is very close. The discussion point is that the interaction and transformed variables increase the complexity of the model, we should consideration if the increased complexity on the model outweights the increased performance, and if the model is easy to explain and implement comparing with the simple model without transformation and interaction term.**
